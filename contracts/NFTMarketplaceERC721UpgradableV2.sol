...
import "./extensions/TokenGovernanceUpgradable.sol";
/**
 ...
 */
contract NFTMarketplaceUpgradableV2 is
    Initializable,
    ...
    ERC721TokenGovernanceUpgradable
{
    ...
    function initialize(
        ...
        __NFTMarketplaceUpgradableV2_init(name, symbol, baseTokenURI, rootAdmin, transferable);
    }
    ...
    function __NFTMarketplaceUpgradableV2_init(
        ...
        __TokenGovernance_init_unchained(transferable);
    }
    ...
    */
    modifier onlyPauser() {
       ...
       /**
    * @dev modifier to check governer rights.
    * contract owner, root admin and governer's have governer rights
    */
      modifier onlyGoverner() {
        require(
            hasRole(ADMIN_ROLE, _msgSender()) || 
            hasRole(GOVERNOR_ROLE, _msgSender()) || 
            owner() == _msgSender(), "Restricted to governer."
            );
        _;
    }
    ...
    */
    ...
    function mint(address receiver,uint collectibleId, string memory IPFSHash, Royalties calldata royalties) public onlyMinter {
    ...
    */
    ...
    function isAddressWhiteListed(address _address) public returns(bool){
        return _isAddressWhiteListed(_address);  
    }

    function isAddressBlackListed(address _address)public returns(bool) {
        return _isAddressBlackListed(_address);
        
    }

    function isTokenBlackListed(uint _tokenid) public returns(bool) {
        return _isTokenBlackListed(_tokenid);
    }

    function isTokenWhiteListed(uint _tokenid) public returns(bool) {
        return _isTokenWhiteListed(_tokenid);
    }

    function isTransferable()public returns(bool){
        return _isTransferable();
    }

    function whiteListAddress(address _address) public onlyGoverner {
        _whiteListAddress(_address);
    }
    
    function blackListAddress(address _address) public onlyGoverner {
        _blackListAddress(_address);
    }

    function whiteListToken(uint _tokenid)public onlyGoverner {
        _whiteListToken(_tokenid);
    }

    function blackListToken(uint _tokenid)public onlyGoverner {
        _blackListToken(_tokenid);
    }

    function removeWhiteListAddress(address _address) public onlyGoverner {
        _removeWhiteListAddress(_address);
    }

    function removeBlackListAddress(address _address) public onlyGoverner {
        _removeBlackListAddress(_address);
    }

    function removeWhiteListToken(uint _tokenid)public onlyGoverner {
        _removeWhiteListToken(_tokenid);
    }

    function removeBlackListToken(uint _tokenid)public onlyGoverner {
        _removeBlackListToken(_tokenid);
    }

    function setTransferable(bool _transferable)public onlyGoverner {
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal virtual override(ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721PausableUpgradeable, ERC721TokenGovernanceUpgradable) {
